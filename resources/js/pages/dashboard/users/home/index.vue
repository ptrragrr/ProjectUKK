...
const countdownText = ref('⏳ Loading countdown...')
const eventDate = new Date('2025-10-12T16:00:00+07:00')
const artists = [
  'Pamungkas', 'Nadin Amizah', 'Reality Club', 'Hindia', 'Barasuara', '+ More'
]
...
const updateCountdown = () => {
  ...
  if (diff <= 0) {
    countdownText.value = "🎉 Festival sedang berlangsung!"
    return
  }

  const days = Math.floor(diff / (1000 * 60 * 60 * 24))
  const hours = Math.floor((diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60))
  countdownText.value = `🌿 ${days} hari ${hours} jam lagi menuju festival! 🎶`
}

const buyTickets = () => {
  // Bisa redirect atau emit event
...
<template>
  <div class="nature-hero">
    <div class="nature-content">
      <h1 class="nature-title">OURSKY.KONSER</h1>
      ...
      <div class="artists-section">
        ...
        <div class="artists-list">
          <span class="artist-tag" v-for="artist in artists" :key="artist">{{ artist }}</span>
        </div>
      </div>
      ...
      <div class="btn-container">
        <button @click="buyTickets" class="btn-primary">🎟️ Beli Tiket Sekarang</button>
        <!-- <button @click="showInfo" class="btn-secondary">📋 Info Lengkap</button> -->
      </div>
    </div>
  </div>
</template>
