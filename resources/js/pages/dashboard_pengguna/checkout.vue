...
// ✅ Tambahkan variable untuk menyimpan order_id
...
// ✅ Fungsi untuk melepas reservasi
async function releaseReservation(orderId: string | null) {
    if (!orderId) {
        console.warn("Tidak ada order_id untuk release reservation");
        return;
    }

    try {
        console.log("Melepas reservasi untuk order:", orderId);

        await axios.post("/checkout/release", {
            order_id: orderId,
        });

        console.log("Reservasi berhasil dilepas");
    } catch (error) {
        console.error("Error saat release reservation:", error);
    }
}
            ...
            window.snap.pay(data.token, {
                onSuccess: async (result: any) => {
                    ...
                    try {
                        ...
                        currentOrderId.value = null; // Reset order_id
                ...
                onPending: (result: any) => {
                    try {
                        console.log("🔵 onPending dipanggil");
                        console.log("📦 Result:", result);
                        console.log("🔑 Order ID:", result.order_id);

                        const url = `/checkout/pending?order_id=${result.order_id}`;
                        console.log("🔗 Redirect ke:", url);

                        window.location.href = url;
                    } catch (error) {
                        console.error("❌ Error di onPending:", error);
                        alert("Error: " + error);
                    }
                },
                onError: async (result: any) => {
                    ...
                    // ✅ Release reservasi saat error
                    await releaseReservation(
                        result.order_id || currentOrderId.value
                    );
                ...
                onClose: async () => {
                    console.log("Modal pembayaran ditutup oleh user");

                    // ✅ PENTING: Release reservasi saat user tutup popup
                    await releaseReservation(currentOrderId.value);

                    hideLoading();

                    // Optional: Tampilkan notifikasi
                    alert(
                        "Pembayaran dibatalkan. Tiket telah dikembalikan ke stok."
                    );
                },
            });
        } else {
                ...
                <form @submit.prevent="startPayment" id="checkout-form">
                    ...
                    <!-- <div class="payment-info">
                        ...
                        <div class="info-text">
                            ...
                            <span
                                >Transaksi Anda dilindungi dengan enkripsi
                                SSL</span
                            >
                        </div>
                    </div> -->
                ...
                <div v-if="keranjang.length > 0" class="ticket-list">
                    ...
                    >
                        <div class="ticket-details">
                            ...
                            <div class="ticket-type" v-if="item.jenis_tiket">
                                {{ item.jenis_tiket }}
                            </div>
                            <div class="ticket-qty">
                                Jumlah: {{ item.qty }} tiket
                            </div>
                        </div>
